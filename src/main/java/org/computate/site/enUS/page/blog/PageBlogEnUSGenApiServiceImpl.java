...
import org.computate.site.enUS.page.blog.PageBlogEnUSPage;


/**
 * Traduire: false
 **/
public class PageBlogEnUSGenApiServiceImpl implements PageBlogEnUSGenApiService {

	protected static final Logger LOGGER = LoggerFactory.getLogger(PageBlogEnUSGenApiServiceImpl.class);

	protected static final String SERVICE_ADDRESS = "PageBlogEnUSApiServiceImpl";

	protected SiteContexteEnUS siteContexte;

	public PageBlogEnUSGenApiServiceImpl(SiteContexteEnUS siteContexte) {
		this.siteContexte = siteContexte;
		PageBlogEnUSGenApiService service = PageBlogEnUSGenApiService.creerProxy(siteContexte.getVertx(), SERVICE_ADDRESS);
	}
	...
	public void rechercheenuspagePageBlogId(OperationRequest operationRequete, Handler<AsyncResult<OperationResponse>> gestionnaireEvenements) {
		rechercheenuspagePageBlog(operationRequete, gestionnaireEvenements);
	}
	...
	public void rechercheenuspagePageBlog(OperationRequest operationRequete, Handler<AsyncResult<OperationResponse>> gestionnaireEvenements) {
		try {
			RequeteSiteEnUS requeteSite = genererRequeteSiteEnUSPourPageBlog(siteContexte, operationRequete);
			sqlPageBlog(requeteSite, a -> {
				if(a.succeeded()) {
					utilisateurPageBlog(requeteSite, b -> {
						if(b.succeeded()) {
							recherchePageBlog(requeteSite, false, true, "/enUS/blog", c -> {
								if(c.succeeded()) {
									ListeRecherche<PageBlog> listePageBlog = c.result();
									reponse200RechercheEnUSPagePageBlog(listePageBlog, d -> {
										if(d.succeeded()) {
	...
	public void reponse200RechercheEnUSPagePageBlog(ListeRecherche<PageBlog> listePageBlog, Handler<AsyncResult<OperationResponse>> gestionnaireEvenements) {
		try {
			Buffer buffer = Buffer.buffer();
			RequeteSiteEnUS requeteSite = listePageBlog.getRequeteSite_();
			ToutEcrivain w = ToutEcrivain.creer(listePageBlog.getRequeteSite_(), buffer);
			requeteSite.setW(w);
			PageBlogEnUSPage page = new PageBlogEnUSPage();
			SolrDocument pageDocumentSolr = new SolrDocument();

			pageDocumentSolr.setField("pageUri_frFR_stored_string", "/enUS/blog");
			page.setPageDocumentSolr(pageDocumentSolr);
			page.setW(w);
			page.initLoinPageBlogEnUSPage(requeteSite);
			page.html();
	...
	public void erreurPageBlog(RequeteSiteEnUS requeteSite, Handler<AsyncResult<OperationResponse>> gestionnaireEvenements, AsyncResult<?> resultatAsync) {
	...
	public void sqlPageBlog(RequeteSiteEnUS requeteSite, Handler<AsyncResult<OperationResponse>> gestionnaireEvenements) {
		try {
	...
	public RequeteSiteEnUS genererRequeteSiteEnUSPourPageBlog(SiteContexteEnUS siteContexte, OperationRequest operationRequete) {
		return genererRequeteSiteEnUSPourPageBlog(siteContexte, operationRequete, null);
	}

	public RequeteSiteEnUS genererRequeteSiteEnUSPourPageBlog(SiteContexteEnUS siteContexte, OperationRequest operationRequete, JsonObject body) {
	...
	public void utilisateurPageBlog(RequeteSiteEnUS requeteSite, Handler<AsyncResult<OperationResponse>> gestionnaireEvenements) {
		try {
			SQLConnection connexionSql = requeteSite.getConnexionSql();
	...
	public void recherchePageBlog(RequeteSiteEnUS requeteSite, Boolean peupler, Boolean stocker, String classeApiUriMethode, Handler<AsyncResult<ListeRecherche<PageBlog>>> gestionnaireEvenements) {
		try {
			...
			listeRecherche.addFilterQuery("classeNomsCanoniques_indexed_strings:" + ClientUtils.escapeQueryChars("org.computate.site.enUS.page.blog.PageBlog"));
