		...
		user(serviceRequest, SiteRequestEnUS.class, SiteUser.class, "computateorg-enUS-SiteUser", "postSiteUserFuture", "patchSiteUserFuture").onSuccess(siteRequest -> {
			try {
				...
				if(
					...
					eventBus.request("computateorg-enUS-SitePage", json, new DeliveryOptions().addHeader("action", "postSitePageFuture")).onSuccess(a -> {
		...
		try {
			jsonArray.forEach(obj -> {
				futures.add(Future.future(promise1 -> {
					...
					JsonObject context = new JsonObject().put("params", params).put("user", siteRequest.getUserPrincipal());
					JsonObject json = new JsonObject().put("context", context);
					eventBus.request("computateorg-enUS-SitePage", json, new DeliveryOptions().addHeader("action", "putimportSitePageFuture")).onSuccess(a -> {
						promise1.complete();
					}).onFailure(ex -> {
	...
	public Future<ServiceResponse> response200SearchPageSitePage(SearchList<SitePage> listSitePage) {
		...
		try {
			...
			page.promiseDeepSitePagePage(siteRequest).onSuccess(a -> {
				...
				json.put(ComputateConfigKeys.PROJECT_POWERED_BY_URL, config.getString(ComputateConfigKeys.PROJECT_POWERED_BY_URL));
				json.put(ComputateConfigKeys.PROJECT_POWERED_BY_NAME, config.getString(ComputateConfigKeys.PROJECT_POWERED_BY_NAME));
				json.put(ComputateConfigKeys.PROJECT_POWERED_BY_IMAGE_URI, config.getString(ComputateConfigKeys.PROJECT_POWERED_BY_IMAGE_URI));
