...
public class C001GenPage extends C001GenPageGen<CoursePage> {
	...
	protected void _stats(Wrap<SolrResponse.Stats> w) {
		w.o(searchListC001_.getResponse().getStats());
	}
	...
	protected void _query(JsonObject query) {
		...
		for(String fq : Optional.ofNullable(searchListC001_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
			if(!StringUtils.contains(fq, "(")) {
				String fq1 = C001.searchVarC001(StringUtils.substringBefore(fq, ":"));
		...
		for(String sort : Optional.ofNullable(searchListC001_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
			String sort1 = C001.searchVarC001(StringUtils.substringBefore(sort, " "));
	...
	protected void _defaultRangeGap(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListC001_.getFacetRangeGap()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> s.getString("defaultRangeGap")).orElse("+1DAY")));
	}

	@Override
	protected void _defaultRangeEnd(Wrap<ZonedDateTime> w) {
		w.o(Optional.ofNullable(searchListC001_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Instant.parse(s.getString("defaultRangeEnd")).atZone(defaultTimeZone)).orElse(ZonedDateTime.now(defaultTimeZone).toLocalDate().atStartOfDay(defaultTimeZone).plusDays(1))));
	}

	@Override
	protected void _defaultRangeStart(Wrap<ZonedDateTime> w) {
		w.o(Optional.ofNullable(searchListC001_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Instant.parse(s.getString("defaultRangeStart")).atZone(defaultTimeZone)).orElse(defaultRangeEnd.minusDays(7).toLocalDate().atStartOfDay(defaultTimeZone))));
	}

	@Override
	protected void _defaultRangeVar(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListC001_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return C001.searchVarC001(v); }).orElse("created"));
	}
	...
	protected void _defaultFieldListVars(List<String> l) {
		Optional.ofNullable(searchListC001_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
			...
			for(String part : parts) {
				if(StringUtils.isNotBlank(part)) {
					String var = C001.searchVarC001(part);
	...
	protected void _defaultStatsVars(List<String> l) {
		Optional.ofNullable(searchListC001_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
