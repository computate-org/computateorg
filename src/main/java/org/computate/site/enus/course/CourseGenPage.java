...
public class CourseGenPage extends CourseGenPageGen<ArticlePage> {
	...
	protected void _pageResponse(Wrap<String> w) {
		if(searchListCourse_ != null)
			w.o(JsonObject.mapFrom(searchListCourse_.getQueryResponse()).toString());
	}
	...
	protected void _defaultRangeGap(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListCourse_.getFacetRangeGap()).orElse("+1DAY"));
	}

	protected void _defaultRangeEnd(Wrap<ZonedDateTime> w) {
		w.o(Optional.ofNullable(searchListCourse_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(ZonedDateTime.now(defaultTimeZone).toLocalDate().atStartOfDay(defaultTimeZone).plusDays(1)));
	}

	protected void _defaultRangeStart(Wrap<ZonedDateTime> w) {
		w.o(Optional.ofNullable(searchListCourse_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(defaultRangeEnd.minusDays(7).toLocalDate().atStartOfDay(defaultTimeZone)));
	}
	...
	protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
		w.o(searchListCourse_.getQueryResponse().getFacetCounts());
	}
	...
	protected void _pagination(JsonObject pagination) {
		...
		Long foundNum = searchListCourse_.getQueryResponse().getResponse().getNumFound().longValue();
	...
	protected void _query(JsonObject query) {
		...
		Long num = searchListCourse_.getQueryResponse().getResponse().getNumFound().longValue();
		String q = "*:*";
