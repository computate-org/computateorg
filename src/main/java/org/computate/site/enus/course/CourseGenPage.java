 ...
 * Translate: false
 **/
public class CourseGenPage extends CourseGenPageGen<ArticlePage> {

	/**
	 * {@inheritDoc}
	 * Ignore: true
	 **/
	protected void _searchListCourse_(Wrap<SearchList<Course>> w) {
	}

	@Override
	protected void _pageResponse(Wrap<String> w) {
		if(searchListCourse_ != null)
			w.o(JsonObject.mapFrom(searchListCourse_.getResponse()).toString());
	}
	...
	protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
		w.o(searchListCourse_.getResponse().getFacetCounts());
	}

	@Override
	protected void _pagination(JsonObject pagination) {
		JsonArray pages = new JsonArray();
		Long start = searchListCourse_.getStart().longValue();
		Long rows = searchListCourse_.getRows().longValue();
	...
	protected void _varsQ(JsonObject vars) {
		Course.varsQForClass().forEach(var -> {
			...
			json.put("val", Optional.ofNullable(searchListCourse_.getRequest().getQuery()).filter(fq -> fq.startsWith(Course.varIndexedCourse(var) + ":")).map(s -> StringUtils.substringAfter(s, ":")).orElse(null));
			vars.put(var, json);
		});
	}
	...
	protected void _varsFq(JsonObject vars) {
		...
		Course.varsFqForClass().forEach(var -> {
			String varIndexed = Course.varIndexedCourse(var);
			String varStored = Course.varStoredCourse(var);
			JsonObject json = new JsonObject();
			json.put("var", var);
	...
	protected void _varsRange(JsonObject vars) {
		Course.varsRangeForClass().forEach(var -> {
			String varIndexed = Course.varIndexedCourse(var);
			JsonObject json = new JsonObject();
			json.put("var", var);
			json.put("displayName", Optional.ofNullable(Course.displayNameCourse(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
	...
	protected void _query(JsonObject query) {
		...
		Long num = searchListCourse_.getResponse().getResponse().getNumFound().longValue();
		String q = "*:*";
		...
		Long rows1 = Optional.ofNullable(searchListCourse_).map(l -> l.getRows()).orElse(10L);
		Long start1 = Optional.ofNullable(searchListCourse_).map(l -> l.getStart()).orElse(1L);
	...
	protected void _defaultRangeGap(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListCourse_.getFacetRangeGap()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> s.getString("defaultRangeGap")).orElse("+1DAY")));
	}

	@Override
	protected void _defaultRangeEnd(Wrap<ZonedDateTime> w) {
		w.o(Optional.ofNullable(searchListCourse_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Instant.parse(s.getString("defaultRangeEnd")).atZone(defaultTimeZone)).orElse(ZonedDateTime.now(defaultTimeZone).toLocalDate().atStartOfDay(defaultTimeZone).plusDays(1))));
	}

	@Override
	protected void _defaultRangeStart(Wrap<ZonedDateTime> w) {
		w.o(Optional.ofNullable(searchListCourse_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Instant.parse(s.getString("defaultRangeStart")).atZone(defaultTimeZone)).orElse(defaultRangeEnd.minusDays(7).toLocalDate().atStartOfDay(defaultTimeZone))));
	}

	@Override
	protected void _defaultRangeVar(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListCourse_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return Course.searchVarCourse(v); }).orElse("created"));
	}

	@Override
	protected void _defaultFacetSort(Wrap<String> w) {
		w.o(Optional.ofNullable(searchListCourse_.getFacetSort()).orElse("index"));
	}

	@Override
	protected void _defaultFacetLimit(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListCourse_.getFacetLimit()).orElse(1));
	}

	@Override
	protected void _defaultFacetMinCount(Wrap<Integer> w) {
		w.o(Optional.ofNullable(searchListCourse_.getFacetMinCount()).orElse(1));
	}
	...
	protected void _courseCount(Wrap<Integer> w) {
		w.o(searchListCourse_ == null ? 0 : searchListCourse_.size());
	}

	protected void _course_(Wrap<Course> w) {
		if(courseCount == 1)
			w.o(searchListCourse_.get(0));
	}
	...
	protected void _pageTitle(Wrap<String> c) {
		if(course_ != null && course_.getObjectTitle() != null)
			c.o(course_.getObjectTitle());
		else if(course_ != null)
			c.o("courses");
		else if(searchListCourse_ == null || courseCount == 0)
			c.o("no course found");
		else
			c.o("courses");
	}
