...
/**
 * Translate: false
 * CanonicalName.frFR: org.computate.site.enus.course.CourseFrFRGenApiServiceImpl
 **/
public class CourseEnUSGenApiServiceImpl extends BaseApiServiceImpl implements CourseEnUSGenApiService {
		...
		try {
			...
			if(facetPivot != null) {
				...
				for(SolrResponse.Pivot pivot : facetPivot.getPivotMap().values()) {
					...
					responsePivotSearchPageCourse(pivot.getPivotList(), pivotArray);
				}
			}
		...
		try {
			...
			if(facetPivot != null) {
				...
				for(SolrResponse.Pivot pivot : facetPivot.getPivotMap().values()) {
					...
					JsonArray pivotArray = new JsonArray();
					facetPivotJson.put(StringUtils.join(entityVars, ","), pivotArray);
					responsePivotSearchCourse(pivot.getPivotList(), pivotArray);
				}
			}
			if(exceptionSearch != null) {
				json.put("exceptionSearch", exceptionSearch);
	...
	public Future<Void> persistCourse(Course o) {
