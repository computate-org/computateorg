...
public class ArticleEnUSGenApiServiceImpl extends BaseApiServiceImpl implements ArticleEnUSGenApiService {
				...
				siteRequest.setJsonObject(body);
				{
					searchArticleList(siteRequest, true, false, true).onSuccess(listArticle -> {
						try {
							...
							if(listArticle.getQueryResponse().getResponse().getNumFound() > 1
								...
								apiRequest.setNumFound(listArticle.getQueryResponse().getResponse().getNumFound());
	...
	public Future<Void> listPATCHArticle(ApiRequest apiRequest, SearchList<Article> listArticle) {
		...
		CompositeFuture.all(futures).onSuccess( a -> {
			if(apiRequest != null) {
				apiRequest.setNumPATCH(apiRequest.getNumPATCH() + listArticle.getQueryResponse().getResponse().getDocs().size());
				if(apiRequest.getNumFound() == 1L)
			...
			try {
				...
				searchArticleList(siteRequest, false, true, true).onSuccess(listArticle -> {
					try {
						...
						if(o != null && listArticle.getQueryResponse().getResponse().getNumFound() == 1) {
							ApiRequest apiRequest = new ApiRequest();
		...
		user(serviceRequest, SiteRequestEnUS.class, SiteUser.class, "computate.org-enUS-SiteUser", "postSiteUserFuture", "patchSiteUserFuture").onSuccess(siteRequest -> {
			try {
				siteRequest.setJsonObject(body);
				{
					...
					eventBus.request("computate.org-enUS-Article", json, new DeliveryOptions().addHeader("action", "postArticleFuture")).onSuccess(a -> {
		...
		try {
			jsonArray.forEach(obj -> {
				futures.add(Future.future(promise1 -> {
					...
					JsonObject context = new JsonObject().put("params", params).put("user", siteRequest.getUserPrincipal());
					JsonObject json = new JsonObject().put("context", context);
					eventBus.request("computate.org-enUS-Article", json, new DeliveryOptions().addHeader("action", "putimportArticleFuture")).onSuccess(a -> {
						promise1.complete();
					}).onFailure(ex -> {
